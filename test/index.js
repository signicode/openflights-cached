...
assert.doesNotReject(async () => {
    ...
    await assert.doesNotReject(async () => {
        ...
        // @ts-ignore
        if (mem1 - mem0 > 5000000) throw new Error(`Large data should not be loaded with main module mem0=${si.bin.format(mem0)}, mem1=${si.bin.format(mem1)}, diff=${si.bin.format(mem1)-mem0}`);
        // @ts-ignore
        console.error(`✔ lazy memory usage test passed usage=${si.bin.format(mem1-mem0)}`);

        // @ts-ignore
        if (mem2 - mem1 < 10000000) throw new Error(`Large data should load on direct exports mem1=${si.bin.format(mem1)}, mem2=${si.bin.format(mem2)}, diff=${si.bin.format(mem2)-mem1}`);
        // @ts-ignore
